...
def _read_private_key_string() -> str:
    ...
    if not pk:
        raise EnvironmentError("Missing private key env var. Set SOLANA_PRIVATE_KEY or WALLET_PRIVATE_KEY(_BASE58).")
    return pk.strip()
...
def jupiter_swap(input_mint: str, output_mint: str, amount: int, wallet: Keypair, slippage_bps: int = DEFAULT_SLIPPAGE_BPS):
    try:
        ...
        if not tx_b64:
            return {"success": False, "signature": None, "error": f"No swapTransaction returned: {str(swap_data)[:200]}"}
...
def get_wallet_summary():
    try:
        ...
        return {"error": str(e), "live_trading": is_live_trading_enabled(), "max_per_trade": get_max_sol_per_trade()}
...
def buy_token(mint_address: str, sol_amount: float):
    ...
    if bal is not None and (bal - sol_amount) < MIN_SOL_RESERVE:
        return {"success": False, "signature": None, "error": f"Insufficient SOL reserve. Balance={bal} reserve={MIN_SOL_RESERVE}"}
